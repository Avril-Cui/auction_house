...
class BotTwo:
    ...
    def trade_stock(self):
        ...
        current_price = json.loads(requests.request("POST", "http://127.0.0.1:5000/current-price", data=payload).text)["price"]

        buy_data = {
            "user_uid": "bot_two",
            "comp_name": self.comp_name,
            "share_number": 100,
            "target_price": float(current_price)-1
        }
    
        reponse = requests.request("POST", self.url, data=buy_data)
        ...
        reponse = requests.request("POST", self.url, data=sell_data)
